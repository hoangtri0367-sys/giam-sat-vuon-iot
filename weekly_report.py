...
def generate_weekly_excel(records: list, week_start: datetime = None) -> bytes:
    """
...
def _sheet_summary(wb, records, week_start, week_end):
    ...
    stats = [
        ("🌱 Số cây TB/lần chụp", f"{avg_plant} / 18"),
...
# ────────────────────────────────────────────────
# SHEET 2: CHI TIẾT TỪNG LẦN CHỤP
# ────────────────────────────────────────────────
...
def _sheet_grid_heatmap(wb, records):
    ...
    for table_idx, (title, metric) in enumerate([
        ...
        ws.merge_cells(f"B{row_base}:G{row_base}")
