...
void loop() {
  ...
  if (now - lastSensorCheck >= SENSOR_CHECK_INTERVAL) {
    ...
    if (!webPumpManual) {
      ...
      // MANUAL: vẫn cập nhật lastSoil để hiển thị đúng trên dashboard, nhưng không tự tưới
      lastSoil = readSoil();
    }
  }

  // (2) Cập nhật state machine của bơm mỗi vòng lặp (bắt đúng thời điểm 5s/3s kết thúc)
  //     Chỉ chạy khi AUTO — nếu đang MANUAL, bơm do web điều khiển trực tiếp, không qua state machine.
  if (!webPumpManual) {
