...
Nhận ảnh top-down từ ESP32-CAM (chụp từ 42cm), chậu 40×40cm.
Layout: 16 cây, 4 hàng × 4 cột.

Tích hợp vào Flask: gọi analyze_image(image_bytes, growth_stage="cay_con") -> dict
"""
...
def _vn_now():
    """Giờ hiện tại theo múi giờ Việt Nam (UTC+7), dạng naive (không kèm offset) để
    đồng nhất định dạng timestamp với phần còn lại của hệ thống (app.py cũng dùng kiểu này).
...
TRAY_WIDTH_CM  = 40.0
TRAY_HEIGHT_CM = 40.0
CAMERA_HEIGHT_CM = 42.0   # camera cách mặt đất 42cm

# Lưới cây
ROWS = 4
COLS = 4
TOTAL_PLANTS = ROWS * COLS  # 16
...
# CHIA THEO GIAI ĐOẠN SINH TRƯỞNG vì lá mầm mới nhú rất nhỏ so với
# cây ở giai đoạn Sinh trưởng/Thu hoạch. Nếu dùng chung 1 ngưỡng cao,
# các ô mới nảy mầm sẽ luôn bị báo "Missing" dù thực tế đã có cây.
#
# Mỗi ô ~10x10cm = 100cm² (tray 40x40cm chia lưới 4x4), nên:
#   0.0015 -> ~0.15cm²  (nay_mam    - lá mầm vừa nhú, chỉ vài mm)
#   0.003  -> ~0.3cm²   (cay_con    - cây nảy chậm hơn cây khác trong khay)
#   0.04   -> ~4cm²     (sinh_truong / thu_hoach - tán lá đã rõ)
#
# Lưu ý: ngưỡng càng thấp càng dễ dính nhiễu (đất vụn, bóng lá cây bên cạnh
# lọt vào ô). Nếu sau khi hạ ngưỡng vẫn thấy báo "có cây" ở ô đất trống,
# tăng nhẹ lại 2 giá trị nay_mam/cay_con hoặc kiểm tra lại morphology filter
# trong _analyze_cell().
CELL_PLANT_THRESHOLD_BY_STAGE = {
    "nay_mam":     0.0015,  # ~0.15cm² — bắt mầm mới nhú (2 lá mầm còn cực nhỏ)
    "cay_con":     0.003,   # ~0.3cm²  — cây con nảy chậm hơn các cây khác trong khay
    "sinh_truong": 0.04,    # ~4cm²
    "thu_hoach":   0.04,    # ~4cm²
}
# Ngưỡng mặc định nếu không truyền growth_stage hoặc stage lạ
CELL_PLANT_THRESHOLD_DEFAULT = 0.04
...
def analyze_image(image_bytes: bytes, growth_stage: str = None) -> dict:
    """
    ...
    growth_stage : str, optional
        Một trong "nay_mam", "cay_con", "sinh_truong", "thu_hoach".
        Quyết định ngưỡng phát hiện cây (ô nhỏ ở giai đoạn nảy mầm vẫn
        được nhận là "có cây" thay vì báo "Missing"). Nếu None hoặc
        không khớp key nào, dùng CELL_PLANT_THRESHOLD_DEFAULT.

    Returns
    -------
    dict với các key:
        timestamp, plant_count, missing_positions,
        cell_results (list 16 phần tử),
        avg_canopy_cm2, health_summary, disease_warning,
        debug_image_bytes (JPEG bytes ảnh đã annotate),
        growth_stage, plant_threshold_used
    """
    # ── 1. Decode ảnh ──────────────────────────────
    ...
    # ── 2. Xác định ngưỡng theo giai đoạn sinh trưởng ──
    plant_threshold = CELL_PLANT_THRESHOLD_BY_STAGE.get(
        growth_stage, CELL_PLANT_THRESHOLD_DEFAULT
    )
    ...
    for r in range(ROWS):
        for c in range(COLS):
            ...
            result = _analyze_cell(cell_img, px_per_cm, plant_threshold, row=r, col=c)
            result["grid_row"] = r + 1
            result["grid_col"] = c + 1
    ...
    return {
        ...
        "plant_threshold_used": plant_threshold,
    }
...
def _analyze_cell(cell_img, px_per_cm: float, plant_threshold: float, row: int, col: int) -> dict:
    """Phân tích một ô trong lưới.

    plant_threshold: ngưỡng green_ratio (0-1) để coi ô là "có cây",
    đã được chọn theo growth_stage ở analyze_image().
    """
    if cell_img.size == 0:
    ...
    has_plant = green_ratio >= plant_threshold
