...
def export_weekly():
    ...
    if not records and not sensor_records:
        return jsonify({"error": f"Không có dữ liệu {days} ngày"}), 404
    xlsx = generate_weekly_excel(records, week_start, sensor_records=sensor_records)
...
def _scheduled_report():
    ...
    if not records and not sensor_records: return
    xlsx  = generate_weekly_excel(records, week_start, sensor_records=sensor_records)
    fname = f"BaoCao_CaiNgot_{week_start.strftime('%d%m%Y')}-{week_end.strftime('%d%m%Y')}.xlsx"
    try:
