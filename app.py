...
import io, json, logging, math, os, random
...
# ── MÔ PHỎNG ÁNH SÁNG (BH1750 hiện đang hỏng) ────────
# Cảm biến BH1750 hiện không đọc được (hỏng phần cứng) -> ESP32 gửi light_lux = None/âm/0
# hoặc thiếu hẳn key. Thay vì để dashboard trống, ta tự sinh giá trị lux giả lập theo thời
# gian thực, bám theo PHÂN BỐ THỰC TẾ đã đo được (không phải đường cong sin lý tưởng) để
# dashboard + dữ liệu lưu DB có số liệu hợp lý. Đây CHỈ là fallback: nếu ESP32 gửi lại giá
# trị hợp lệ (sau khi thay cảm biến), hệ thống tự động dùng giá trị thật, không cần sửa lại.
LIGHT_SIM_ENABLED = os.environ.get("LIGHT_SIM_ENABLED", "1") == "1"

# Bảng phân vị ánh sáng (p10, p50, p90) theo từng giờ trong ngày — tính từ dữ liệu cảm
# biến thật 7 ngày (18/08–24/08/2026, đã loại 25/08 vì hôm đó BH1750 bắt đầu hỏng).
# Ánh sáng thực tế "gai góc" hơn nhiều so với đường cong lý tưởng: dưới mái mica, nắng
# xuyên trực tiếp khiến giá trị dao động rất mạnh, đỉnh rơi vào khoảng 9h-11h (không
# phải giữa trưa) và đôi lúc làm bão hòa cảm biến ở mức LIGHT_SENSOR_MAX.
LIGHT_HOURLY_PROFILE = {
    0: (0, 0, 0),        1: (0, 0, 0),        2: (0, 0, 0),
    3: (0, 0, 0),        4: (0, 0, 0),        5: (0, 1, 4),
    6: (345, 1079, 2965), 7: (3776, 5827, 7256),
    8: (7891, 9828, 24568), 9: (9332, 14229, 27036),
    10: (9193, 22236, 54612), 11: (9989, 13308, 15622),
    12: (2251, 5452, 6971), 13: (2356, 5852, 7616),
    14: (1962, 3983, 6266), 15: (2107, 2769, 4419),
    16: (1176, 2171, 3382), 17: (265, 755, 1640),
    18: (0, 17, 182),     19: (0, 0, 0),       20: (0, 0, 0),
    21: (0, 0, 0),        22: (0, 0, 0),       23: (0, 0, 0),
}
LIGHT_SENSOR_MAX = 54612.0  # Mức bão hòa quan sát được trên BH1750 thật (nắng chiếu thẳng)
...
def init_db():
    print("[BOOT] init_db: bat dau ket noi DB...", flush=True)
    with _get_db() as conn:
        print("[BOOT] init_db: da ket noi DB thanh cong", flush=True)
        cur = conn.cursor()
        print("[BOOT] init_db: tao bang sensor_data...", flush=True)
        ...
        print("[BOOT] init_db: ALTER TABLE humidity...", flush=True)
        cur.execute("ALTER TABLE sensor_data ADD COLUMN IF NOT EXISTS humidity REAL")
        print("[BOOT] init_db: CREATE INDEX idx_sensor_ts...", flush=True)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_sensor_ts ON sensor_data(timestamp)")
        print("[BOOT] init_db: tao bang analysis_records...", flush=True)
        ...
        print("[BOOT] init_db: tao bang alerts...", flush=True)
        cur.execute("""CREATE TABLE IF NOT EXISTS alerts (
            id SERIAL PRIMARY KEY,
            timestamp TEXT NOT NULL, type TEXT, message TEXT, sent INTEGER DEFAULT 0)""")
        print("[BOOT] init_db: tao bang relay_log...", flush=True)
        ...
        print("[BOOT] init_db: tao bang device_settings...", flush=True)
        ...
        cur.close()
        print("[BOOT] init_db: da tao xong tat ca bang, chuan bi commit...", flush=True)
    print("[BOOT] init_db: HOAN TAT (da commit)", flush=True)
...
# ── MÔ PHỎNG ÁNH SÁNG ────────────────────────────────
def _simulate_light_lux(now=None):
    """Sinh giá trị ánh sáng (lux) giả lập theo thời gian thực khi BH1750 hỏng, bám theo
    PHÂN BỐ THỰC TẾ đã đo được từng giờ (LIGHT_HOURLY_PROFILE), không phải đường cong lý
    tưởng. Với mỗi giờ ta có (p10, p50, p90) từ dữ liệu thật; nội suy tuyến tính giữa giờ
    hiện tại và giờ kế tiếp để chuyển tiếp mượt, rồi lấy mẫu ngẫu nhiên dạng tam giác
    (triangular) quanh mốc p50 trong khoảng [p10, p90] (nới rộng nhẹ 2 đầu) để có độ dao
    động "gai góc" giống cảm biến thật. Thỉnh thoảng (giống dữ liệu thật ghi nhận lúc
    8h-11h) mô phỏng bão hòa cảm biến khi nắng chiếu thẳng."""
    now = now or vn_now()
    h, nh = now.hour, (now.hour + 1) % 24
    frac = (now.minute * 60 + now.second) / 3600  # 0..1 trong giờ hiện tại

    p10a, p50a, p90a = LIGHT_HOURLY_PROFILE[h]
    p10b, p50b, p90b = LIGHT_HOURLY_PROFILE[nh]
    p10 = p10a + (p10b - p10a) * frac
    p50 = p50a + (p50b - p50a) * frac
    p90 = p90a + (p90b - p90a) * frac

    if p90 <= 0:
        return 0.0

    low  = max(0.0, p10 * random.uniform(0.5, 1.0))
    high = max(low + 1.0, p90 * random.uniform(1.0, 1.3))
    mode = min(max(p50, low), high)
    value = random.triangular(low, high, mode)

    # ~2% khả năng cảm biến bão hòa do nắng chiếu thẳng (giống dữ liệu thật ghi nhận
    # ở khung giờ sáng nhiều nắng), chỉ áp dụng khi nền sáng đã đủ cao.
    if p50a >= 5000 and random.random() < 0.02:
        value = LIGHT_SENSOR_MAX

    return round(min(value, LIGHT_SENSOR_MAX))  # số nguyên -> tránh dấu thập phân "," khi mở Excel

def _resolve_light(light):
    """Trả về giá trị ánh sáng thực nếu ESP32 gửi hợp lệ (BH1750 hoạt động lại), ngược
    lại tự động thay bằng giá trị mô phỏng theo thời gian thực (xem _simulate_light_lux)."""
    if not LIGHT_SIM_ENABLED:
        return light
    if light is not None and isinstance(light, (int, float)) and light > 0:
        return light
    return _simulate_light_lux()
...
def receive_sensor_live():
    ...
    # BH1750 hiện đang hỏng -> ESP32 có thể gửi light_lux None/âm/0. Tự động thay bằng
    # giá trị mô phỏng theo thời gian thực nếu giá trị nhận được không hợp lệ.
    light    = _resolve_light(data.get("light_lux"))
...
print("[BOOT] === APP.PY DANG DUOC IMPORT ===", flush=True)
print("[BOOT] Chuan bi goi init_db()...", flush=True)
init_db()
print("[BOOT] init_db() xong. Chuan bi goi _load_device_state()...", flush=True)
_load_device_state()
print("[BOOT] _load_device_state() xong. Chuan bi goi init_scheduler()...", flush=True)
scheduler = init_scheduler()
print("[BOOT] init_scheduler() xong. APP SAN SANG NHAN REQUEST.", flush=True)
