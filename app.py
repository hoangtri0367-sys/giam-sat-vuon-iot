...
def request_manual_capture():
    """Dashboard gọi khi bấm nút 'Chụp ảnh ngay' -> đặt cờ để ESP32-CAM (đang ở
    chế độ luôn thức, poll định kỳ) phát hiện và chụp ở lần poll gần nhất."""
    global _manual_capture_requested
...
def check_manual_capture():
    """ESP32-CAM gọi định kỳ (VD mỗi 10-15s) để kiểm tra có yêu cầu chụp thủ công không."""
    return jsonify({"manual_capture": _manual_capture_requested})

@app.route("/api/manual-capture/clear", methods=["POST"])
def clear_manual_capture():
    """ESP32-CAM gọi sau khi đã chụp/gửi xong ảnh thủ công, để xóa cờ tránh chụp lặp lại."""
    global _manual_capture_requested
    _manual_capture_requested = False
